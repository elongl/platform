  ...
  private async processDerived (ctx: MeasureContext<SessionData>, txes: Tx[]): Promise<void> {
    ...
    const triggers = await ctx.with(
      ...
      async (ctx) => {
        ...
        result.push(...transactions, ...applyTxes)
          ...
          const asyncTriggerProcessor = async (): Promise<void> => {
            await ctx.with(
              ...
              async (ctx) => {
                ...
                const aresult = await performAsync(ctx)

                if (aresult.length > 0 || applyTxes.length > 0) {
                  await this.processDerivedTxes(ctx, aresult.concat(applyTxes))
                  // We need to send all to recipients
                  await this.context.handleBroadcast?.(ctx)
                }
              },
              { count: txes.length }
            )
