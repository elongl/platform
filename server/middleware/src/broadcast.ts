...
export class BroadcastMiddleware extends BaseMiddleware implements Middleware {
  constructor (
    ...
    super(context, next)
    context.handleBroadcast = (ctx) => this.doBroadcast(ctx)
  }
  ...
  async handleBroadcast (ctx: MeasureContext, tx: Tx[], targets?: string | string[], exclude?: string[]): Promise<void> {
    ...
    this.broadcast(ctx, tx, targets, exclude)
  }
  ...
  async doBroadcast (ctx: MeasureContext<SessionData>): Promise<void> {
    if (ctx.contextData.broadcast.txes.length === 0) {
      return
    }
    ...
    for (const txd of ctx.contextData.broadcast.txes) {
      ...
      for (const tt of Object.values(ctx.contextData.broadcast.targets ?? {})) {
        target = tt(txd)
        if (target !== undefined) {
          break
        }
