...
export class ClientSession implements Session {
  ...
  async getAccount (ctx: ClientSessionCtx): Promise<void> {
    const account = await this._pipeline.context.modelDb.findAll(core.class.Account, { email: this.token.email })
    if (account.length === 0 && this.token.extra?.admin === 'true') {
      const systemAccount = await this._pipeline.context.modelDb.findAll(core.class.Account, {
  ...
  async findAllRaw<T extends Doc>(
    ...
    return await this._pipeline.findAll(ctx, _class, query, options)
  }
  ...
  async tx (ctx: ClientSessionCtx, tx: Tx): Promise<void> {
    ...
    await this._pipeline.context.handleBroadcast?.(ctx.ctx)
  }
